...
module SetTests = 
    let rec isSetValid n mn mx =
        ...
        | Node(h, v, ln, rn) ->
            let isInBounds = 
                mn |> Option.forall (fun mn -> v > mn) && 
                mx |> Option.forall (fun mx -> v < mx)
    ...
    let ``Right-Left rotation (RL case)`` () =
        let resultSet = 
            Node(2, 15, 
                Node(0, 10, Empty, Empty),
                Node(1, 20, 
                    Node(0, 16, Empty, Empty),
                    Node(0, 24, Empty, Empty))) 
            |> AVLSet.add 27
    ...
    let ``Standard set union`` () =
        ...
        advancedContains (fun v x -> x = true) setB unionSet) |> should be True
    ...
    let ``Standard set intersection`` () =
        ...
        advancedContains (fun v x -> if AVLSet.contains v setB then x = true else x = false) 
            setA intersectionSet |> should be True
    ...
    let ``Standard set difference`` () =
        ...
        advancedContains (fun v x -> if AVLSet.contains v setB then x = false else x = true) 
            setA differenceSet |> should be True
    ...
    let ``Standard symmetric difference`` () =
        ...
        (advancedContains (fun v x -> if AVLSet.contains v setB then x = false else x = true) 
            setA symmDiffSet && 
        advancedContains (fun v x -> if AVLSet.contains v setA then x = false else x = true) 
            setB symmDiffSet) |> should be True
